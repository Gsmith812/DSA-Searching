...
// Then implement inOrder(), preOrder(), and postOrder() functions. Test your 
...
const values = [25, 15, 50, 10, 24, 35, 70, 4, 12, 18, 31, 44, 66, 90, 22];
const BST = new BinarySearchTree();
values.forEach(value => BST.add(value, value));
...
const preOrder = (node, values = []) => {
    ...
    if(node.right) {
        values = preOrder.length(node.right, values);
    }
    return values;
}
